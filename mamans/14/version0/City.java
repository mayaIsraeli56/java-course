...
public class City {
  ...
  public City(
    ...
    final int dayEstablished,
    final int monthEstablished,
    final int yearEstablished,
    final int centerX,
    final int centerY,
    final int stationX,
    final int stationY,
    final long numOfResidents,
    final int noOfNeighborhoods
  ) {
    this._cityName = cityName;
    this._dateEstablished =
      new Date(dayEstablished, monthEstablished, yearEstablished);
    ...
    this._numOfNeighborhoods =
      ((noOfNeighborhoods <= 1) ? 1 : noOfNeighborhoods);
  }
  ...
  public boolean equals(final City other) {
    return (
      this._cityName.equals(other._cityName) &&
      this._dateEstablished.equals(other._dateEstablished) &&
      this._cityCenter.equals(other._cityCenter) &&
      this._centralStation.equals(other._centralStation) &&
      this._numOfResidents == other._numOfResidents &&
      this._numOfNeighborhoods == other._numOfNeighborhoods
    );
  }
  ...
  public boolean cityEstablishedBetweenDates(Date date1, Date date2) {
    ...
    return (
      this._dateEstablished.equals(date1) ||
      this._dateEstablished.equals(date2) ||
      (
        this._dateEstablished.after(date1) &&
        this._dateEstablished.before(date2)
      )
    );
  }
