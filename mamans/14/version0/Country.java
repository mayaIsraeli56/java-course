 ...
 * City is represented by its name, and by the head of a linked list that
 * contains the cities in this country.
 * Id : 209956333
 *
 * @author Maya Israeli
 * @version 05.06.2023
...
public class Country {
     ...
     * It accepts as parameters its name, its establishment date, x and y
    ...
    public City southernmostCity() {
        ...
        while (node != null) {
            if (southernmost.getCityCenter().isAbove(node.getCity().getCityCenter())) {
    ...
    public boolean wereCitiesEstablishedBeforeOrAfter(Date date1, Date date2) {
        ...
        while (node != null) {
            if (node.getCity().getDateEstablished().before(date1) ||
    ...
    private boolean isAfterChronologicallyAndLexically(
            ...
            Date nextCityDate,
            String nextCityName) {
        return (newCityDate.after(nextCityDate) ||
